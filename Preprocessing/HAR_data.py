...
ds = salem.open_wrf_dataset(home +"/Seafile/EBA-CA/Tianshan_data/HARv2/variables/all_variables_kyzylsuu_HARv2_2007_2014.nc")
crop_extent = shdf.to_crs(ds.pyproj_srs)
crop_extent.crs = ds.pyproj_srs

## Test xarray:
ds = xr.open_dataset(home + "/Seafile/EBA-CA/Tianshan_data/HARv2/variables/all_variables_kyzylsuu_HARv2_2007_2014.nc")

# STIMMT DAS? SIND DIE KOORDINATEN WIRKLICH IN GRAD? WIRD DIE RICHTIGE GITTERZELLE ANGESPROCHEN?
pick = ds.sel(south_north=42.191433, west_east=78.200253, method='nearest')           # closest to AWS location
...
#add buffer to geom, otherwise not detected within bounds of 10km spacing
new_shdf.geometry = new_shdf.geometry.buffer(6000)

clipped_ds = ds.salem.subset(shape=new_shdf)

ds_static = salem.open_wrf_dataset(home + '/Seafile/EBA-CA/Tianshan_data/HARv2/static/all_static_kyzylsuu_HARv2_2007_2014.nc')
...
clipped_ds.prcp_nc.mean(dim='time').plot(ax=ax, zorder=-1)
plt.show()
